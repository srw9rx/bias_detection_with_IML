...
for label, inputs in tqdm(enumerate(loader)):
  pred = inputs[0]
  text = inputs[1].split(" ")
  #print(inputs[1], type(inputs[1]))
  #print(vectorizer.transform([inputs[1]]).todense().shape[1])
  #spacytoks = [i.text for i in inputs] #convert back into the original sentence
  if len(text) > 6:
    limeexplanation = limeexplainer.explain_instance(inputs[1], pipeline.predict_proba, num_features=6)
    thisvallime = limeexplanation.as_list()
  else: 
    thisvallime = 'delete'
  #print(thisvallime)
  thisvalshap = []
  for idx in range(len(text)):
    ...
    #thisvallime.append(limeexplanation)
...
testdata.to_csv(os.path.join(path+'withlime.tsv'), sep='\t', header=0)